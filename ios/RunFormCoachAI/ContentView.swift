...
struct ContentView: View {
    var body: some View {
        TabView {
            AnalyzeView()
                .tabItem { Label("Analyze", systemImage: "video.badge.waveform") }
...
struct AnalyzeView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Running video → strength plan")
                .font(.title2)
                .fontWeight(.semibold)
            Text("Upload a short side-view running video. RunForm analyzes movement and recommends drills.")
            ...
            .overlay {
                VStack(spacing: 8) {
                    ...
                    Text("No video selected")
                        .font(.headline)
                    Text("Use a 10–20 second side-view clip for best results.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    private func analyzeSelectedVideo() async {
        ...
        do {
            ...
            errorMessage = "Analysis failed. Check backend URL and Railway status."
        }
        isAnalyzing = false
    }
}
